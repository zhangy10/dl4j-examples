...
public class ZacDQN implements DQNFactory {
    ...
    public DQN buildDQN(int[] numInputs, int numOutputs) {
        ...
        NeuralNetConfiguration.ListBuilder confB = (new NeuralNetConfiguration.Builder()).seed(12345L).optimizationAlgo(OptimizationAlgorithm.STOCHASTIC_GRADIENT_DESCENT).updater((IUpdater)(this.conf.getUpdater() != null ? this.conf.getUpdater() : new Adam())).weightInit(WeightInit.XAVIER).l2(this.conf.getL2()).list().layer(0, ((org.deeplearning4j.nn.conf.layers.DenseLayer.Builder)((org.deeplearning4j.nn.conf.layers.DenseLayer.Builder)((org.deeplearning4j.nn.conf.layers.DenseLayer.Builder)(new org.deeplearning4j.nn.conf.layers.DenseLayer.Builder()).nIn(nIn)).nOut(this.conf.getNumHiddenNodes())).activation(Activation.RELU)).build());
    ...
    public ZacDQN(DQNFactoryStdDense.Configuration conf) {
        this.conf = conf;
    }
