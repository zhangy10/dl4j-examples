...
public class TrainChain extends Thread {
...
//    private INDArray w0 = null;
    ...
    public TrainChain(BlockingQueue<Msg> sendQueue, int id, Config settings, boolean isLinked, SplitListener splitListener, String modelFile) {
        this(sendQueue, id, settings, splitListener, modelFile);
        this.isLinked = isLinked;
    }

    public TrainChain(BlockingQueue<Msg> sendQueue, int id, Config settings, SplitListener splitListener, String modelFile) {
    ...
    public TrainChain(int id, Config settings, int slaveNum, boolean isLinked, SplitListener splitListener, String modelFile) {
        this(id, settings, slaveNum, splitListener, modelFile);
        this.isLinked = isLinked;
    }

    public TrainChain(int id, Config settings, int slaveNum, SplitListener splitListener, String modelFile) {
        ...
        private void sync(Model model) {
            ...
            if (isMaster) {
                ...
                if (isLinked && slaveNum > 0) {
                    ...
                    newMsg.parameters.muli(getScale());
//                    newMsg.parameters.divi(getScale());
...
//                            newMsg.parameters.divi(getScale());
                            ...
                            System.out.println("No need scale as leaf node....... " + id);
                        }
        ...
        switch (SystemRun.layerConfig) {
            ...
            case LSTM:
                builder = builder.layer(4, lstm("l1", config.getF1_out(), config.getNonZeroBias(), config.getDropOut()))
                              ...
                              .layer(5, new OutputLayer.Builder().activation(Activation.SOFTMAX)
